...
void CWndTracer(uint32_t oldCwnd, uint32_t newCwnd) {
	if(Simulator::Now().GetSeconds() >= 4 && Simulator::Now().GetSeconds() <= 6){
...
int main(int argc, char *argv[]) {
	...
	// Config::SetDefault ("ns3::TcpSocketBase::MaxWindowSize", UintegerValue(MAX RWIN));

	//TODO print here the tcp socket type and the droptail Queue Size
	std::cerr << "TCP socket type: " << tcpSocketType << endl;
	std::cerr << "Droptail Queue Size: " << droptailQueueSize << " packets" << endl;
	...
	std::cerr << "Client address : " << clientIPAddress << endl;
	std::cerr << "Server address : " << serverIPAddress << endl;
    ...
    Address sinkAddress(InetSocketAddress(serverIPAddress, sinkPort));
	...
	clientApp->Setup(clientSocket, sinkAddress, tcpPacketSize, DataRate(datarate));
