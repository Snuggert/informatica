...
ray_trace(void)
{
    vec3    forward_vector, right_vector, up_vector, color, direction;
    int     i, j, width_division, height_division;
    float   image_plane_width, image_plane_height, pixel_height, pixel_width,
            horizontal_direction, vertical_direction, alias_horizontal,
            alias_vertical;
    char    buf[128];
    ...
    /*
     * Determine pixelheight and pixelwidth.
     */
    pixel_width = image_plane_width / framebuffer_width;
    pixel_height = image_plane_height / framebuffer_height;
    ...
    for (j = 0; j < framebuffer_height; j++)
    {
        ...
        {
            horizontal_direction = -(image_plane_width / 2) +
                    (i * pixel_width) - (pixel_width * 0.5);
            vertical_direction = (image_plane_height / 2) -
                    (j * pixel_height) - (pixel_height * 0.5);
            if(do_antialiasing){
                color = v3_create(0, 0, 0);
                for(width_division = 0; width_division < 2;
                        width_division ++)
                {
                    for(height_division = 0; height_division < 2;
                        height_division ++)
                    {
                        horizontal_direction = -(image_plane_width / 2) +
                            (i * pixel_width) - (pixel_width * 0.5) +
                            pixel_width * 0.5 * width_division;
                        vertical_direction = (image_plane_height / 2) -
                            (j * pixel_height) - (pixel_height * 0.5) +
                            pixel_height * 0.5 * height_division;
                        direction = v3_add(forward_vector, v3_add(
                                v3_multiply(right_vector, horizontal_direction),
                                v3_multiply(up_vector, vertical_direction)));

                        color = v3_add(color, ray_color(0,
                                scene_camera_position, direction));
                    }
                }
