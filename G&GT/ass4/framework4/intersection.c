    ...
    vec3 direction, float t0, float t1)
{
    ...
    /* If the node is a leafnode, try to find the nearest matching triangle */
    if(node.is_leaf)
    {
        int index, result;
        result = 0;
        triangle *box_triangles;
        box_triangles = leaf_node_triangles(&node);
        for(index = 0; index < leaf_node_num_triangles(&node); index++)
        {
            if(ray_intersects_triangle(ip, box_triangles[index], origin,
                    direction))
                result = 1;
        }
        return result;
    ...
    /* When both nodes are intersected check which one is closest */
    if(left && right)
    {
        intersection_point *ip1 = malloc(sizeof(intersection_point));
        intersection_point *ip2 = malloc(sizeof(intersection_point));
        ...
        if(left && right)
        {
            /* pick node with closest intersection */
            if(ip1->t > ip2->t){
                *ip = *ip2;
            }
            else{
    ...
    /* Try to find the closest triangle from the left node. */
    else if(left){
        return bvh_traversal(leftnode, ip, origin, direction, tl_min, tl_max);
    }

    /* Try to find the closest triangle from the right node. */
    else if(right){
