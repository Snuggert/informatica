...
update_curve_intersections(void)
{
    ...
    {
        if (intersect_cubic_bezier_curve(animation_time, &current_rotation_speed, &(control_points[0][3*c]), NUM_LINES))
            break;
    }

    for (c = 0; c < NUM_BEZIER_CURVES_PER_LINE; c++)
    {
        if (intersect_cubic_bezier_curve(animation_time, &current_joint1_speed, &(control_points[1][3*c]), NUM_LINES))
            break;
    }

    for (c = 0; c < NUM_BEZIER_CURVES_PER_LINE; c++)
    {
        if (intersect_cubic_bezier_curve(animation_time, &current_joint2_speed, &(control_points[2][3*c]), NUM_LINES))
            break;
    }

    for (c = 0; c < NUM_BEZIER_CURVES_PER_LINE; c++)
    {
        if (intersect_cubic_bezier_curve(animation_time, &current_grabber_speed, &(control_points[3][3*c]), NUM_LINES))
            break;
    }
}
